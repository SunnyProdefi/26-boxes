...
音效与动画由 `src/app.js` 和 `styles.css` 共同承担，不进入 `src/game-engine.js`，以保持规则引擎可测试。

实现要点：

- Web Audio：通过浏览器 `AudioContext` 合成短音效，不依赖外部音频文件。
- 音效开关：使用 `localStorage` 保存玩家偏好，默认开启，玩家可在顶部切换。
- 动画触发：浏览器层在状态迁移完成并重新渲染后，为对应 DOM 节点短暂添加动画 class。
- 开箱反馈：触发箱子弹开、金额划除和开箱音效。
- 报价反馈：触发银行家来电动画和铃声音效。
- 终局反馈：触发中奖箱高亮、结果面板动画和奖花粒子。
- 降低动态效果：CSS 使用 `prefers-reduced-motion` 降低动画强度。
